...
//Tank::Tank(int pin, tN2kFluidType fluidType, int instance, float capacity, float voltage, float resistor, float resistor_tank_max,float &resistance_mapping[][])
{
      ...
      if (R1 >= _resistor_tank_max * 0.99)
      {
        ...
        for (int i = _length - 2; i >= 0; i--)
        {
          if (R1 > _resistance_mapping[i][0]) // The measured resistance R1 is between the mapped resistances on place i and i+1;
          {
            ...
            float xDiff = std::max(_resistance_mapping[i + 1][0] - _resistance_mapping[i][0], 0.001f); // Avoid divison by zero
